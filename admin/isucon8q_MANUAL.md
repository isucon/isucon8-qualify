...
[予選レギュレーション(TODO:リンクを直す)](http://isucon.net/archives/50927022.html) も併せてご確認ください。
...
サーバーには連番の名前がついており、初期構成では先頭の1つがWeb & DBサーバー、残りの2つが何もしないサーバーになっています。

各サーバーにはNICが2つ接続されていて、それぞれグローバルIPとローカルIPが割り振られています。
IPアドレスはポータルサイトから確認することができます。

競技者がサーバーに接続して作業を行う際や、動作確認を行う際は **グローバルIP** を使用することができます。
また、ベンチマークは **ローカルIP** に対して実行されますのでご注意ください。

ローカルIP間の通信にはスイッチによる帯域制限 (100Mbps) がかけられています。
グローバルIP側の帯域は100Mbpsです。
...
サーバーはパスワード認証になっていて、ユーザー名は `isucon` 、パスワードはポータルのパスワードと同一です。

サーバーには予選終了後の主催者による確認作業(追試)のため、 `isucon-admin` ユーザーのアカウントがあります。
`isucon-admin` ユーザーのアカウントに関して、アカウントの削除や既存の公開鍵の削除を行ったことにより主催者による追試をおこなうことができない場合、失格とします。
...
また、PHPを使う場合は、 `systemd` の設定変更の他に、次のように `h2o` の設定ファイルの変更が必要です。

```sh
$ sudo mv /etc/h2o/h2o.conf     /etc/h2o/h2o.conf.backup
$ sudo mv /etc/h2o/php-h2o.conf /etc/h2o/h2o.conf
...
`~/torb/db` ディレクトリに、スキーマを定義した `schema.sql` ファイルと、初期データセットが入った `isucon8q-initial-dataset.sql.gz` ファイルがあります。
...
`~/torb/db` ディレクトリと `~/torb/webapp` ディレクトリは競技用の全サーバーに存在するので、間違って重要なファイルを消してしまっても他のサーバーからリカバリすることができます。
...
(TODO:今は負荷走行後に postTest (全体レポートの確認) もしていて 60秒 許容していますが、なくても良い気もしてきていたのを思い出した。)
...
しかし、過去5秒以内に何らかのエラーが発生していた場合は負荷レベルが上昇しません。
...
(TODO: これは去年のままなので計算式直す)
**成功したGETリクエスト数＋成功したPOSTリクエスト数×3＋受信したメッセージ件数**

しかし、以下の注意事項があります。

(TODO:リダイレクトの仕様はどうあるべきなのか確認が必要)
- 静的ファイルに対するアクセスは、100件で1点とします。
...
ただし、静的コンテンツや画像ファイルに関しては、HTTPの規則の範囲内でステータスコード200の代わりに304を返すことができます。
...
またこの処理が10秒以上レスポンスを返さない場合、失格とします。
...
時間内に登録作業が行われなかった場合には失格となりますのでご注意ください。
